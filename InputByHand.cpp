...
std::sort(A.paz().begin(), A.paz().end()); //pažymiai surikiuojami, kad būtų galima apskaičiuoti medianą

int kiekis = A.paz().size(); //skaičiuojama, kiek yra pažymių

if(kiekis == 0) A.mediana() = 0; //jei pažymių nėra - mediana lygi 0
else if(kiekis % 2 == 0) A.mediana() = (A.paz()[kiekis/2 - 1] + A.paz()[kiekis/2]) / 2.0; //jei pazymiu skaicius yra lyginis - skaičiuojama mediana kaip dvieju viduriniu skaiciu vidurkis
